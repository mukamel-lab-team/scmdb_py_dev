...
function initDataTable() {
    ...
    table = $('#dataset-table').DataTable( {
        ...
        "pageLength": 25,
        "ajax": {
            "url": $SCRIPT_ROOT+"/content/datasets/all",
            "dataSrc": ""
        },
        "columns": [
            {
...
function verifySimilarity() {
    ...
    $.getJSON({
        url: $SCRIPT_ROOT+'/content/check_ensembles/'+$("#ensemble-name-input").val().trim()+'/'+$("#selected-datasets").val().replace(/, /g, '+'),
...
$("#final-submit-button").on('click', function() {
    ...
    $.getJSON({
        url: $SCRIPT_ROOT+'/submit_new_ensemble/'+$("#ensemble-name-input").val().trim()+'/'+$("#selected-datasets").val().replace(/, /g, '+')+'?description='+$("#ensemble-description").val(),
        success: function(data) {
            if (data['result'] === true) {
